...
Two sortable columns (`total_performance_score`, `ed_boarding_time_minutes`) are nullable, because `mart_ed_performance_vs_hvbp` is built with left joins and not every hospital has both an ED and an HVBP record. NULLs are ordered last in both sort directions, and the keyset predicate branches explicitly on whether the cursor's last row was itself in that NULL bucket, rather than trusting a single row-value comparison across a NULL boundary — a naive `(col, ccn) > (?, ?)` comparison silently drops every remaining row once either side is NULL. Pagination completeness across that NULL boundary, in both directions, is proven by `test_list_hospitals_pagination_is_complete_with_nullable_sort_field`, not just asserted.
